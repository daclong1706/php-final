...
<form action="{{ route('admin.courses.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md max-w-lg">
    ...
    <div class="mb-4">
        ...
        <input type="text" name="grade" id="grade" class="w-full p-3 border rounded-lg focus:ring focus:ring-indigo-300 @error('grade') border-red-500 @enderror" value="{{ old('grade') }}">
        @error('grade')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
    <div class="mb-4">
        ...
        <input type="number" step="0.01" name="price" id="price" class="w-full p-3 border rounded-lg focus:ring focus:ring-indigo-300 @error('price') border-red-500 @enderror" value="{{ old('price') }}">
        @error('price')
        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        @enderror
    </div>
